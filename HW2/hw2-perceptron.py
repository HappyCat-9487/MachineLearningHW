# -*- coding: utf-8 -*-
"""
Created on Fri Nov 29 00:23:48 2019
...
#導入資料
data=pd.read_csv('perceptron2.csv',header=None)
#分配資料
x1, x2, y=data.iloc[0:240, 0].values, data.iloc[0:240, 1].values, data.iloc[0:240, 2].values
#分配train資料集及test資料集
train_x1=x1
train_x2=x2
train_y=y
...
plt.text(-4,0,'%.2f percent' % accuracy, fontdict={'size': 20, 'color':  'green'})      
plt.show()

#testing cross entropy計算
cross_entropy_1=0
...
plt.text(-4,0,'%.2f percent' % accuracy1, fontdict={'size': 20, 'color':  'green'})   
plt.show()